...
public class RayTracedPlane : MonoBehaviour
{
    ...
    public RayTracer.Plane GetPlane()
    {
        // Careful to handle colorspaces here.
        var albedo = GetComponent<MeshRenderer>().material.color.linear;
        albedo *= ColorMultiplier;

        var plane = new RayTracer.Plane();
        plane.Albedo = new Vector3(albedo.r, albedo.g, albedo.b);
        plane.Scale = transform.localScale.x;
        plane.Center = transform.position;
        plane.Material = (int)Material;
